...
\textbf{The Mechanistic Fixed-Effects Topic Model: Recovering Corporate Culture from Employee Reviews}\\
\textit{with \href{https://dan-biderman.netlify.app/}{Dan Biderman}, \href{https://www.cs.columbia.edu/~blei/}{David Blei}, \href{https://business.columbia.edu/faculty/people/wei-cai}{Wei Cai}, \href{https://www.amirfeder.com/}{Amir Feder} and \href{https://www.columbia.edu/~ap3116/}{Andrea Prat}}

\medskip

\textit{Abstract:} Social scientists use topic models to compare documents produced by different authors and in different contexts, even though the same broad topic may be expressed differently across environments. Existing methods usually control for this variation ex post and cannot separate common themes from environment-specific language. Their reliance on word counts also misses concepts conveyed through phrases, context, and tone. We introduce the mechanistic Fixed Effects Topic Model (mFETM), which embeds fixed effects in the generative process and replaces word counts with interpretable concepts extracted from a large language model using sparse autoencoders (SAEs). Applied to a large corpus of Glassdoor reviews, mFETM recovers topics that are more stable across observed environments without sacrificing fit. At the firm level, mFETM recovers a broad, interpretable set of workplace dimensions associated with employer quality. A conventional model instead mixes these dimensions with environment-specific content, such as technology strategy, producing a narrower view of what distinguishes top employers. The removal exercise makes this difference visible: mFETM retains substantially more of its original fit when environment-associated topics are removed. Finally, the SAE representation lets us generate counterfactual reviews by steering along estimated fixed-effect directions, holding the topic fixed while changing its environment.

\vspace{10pt}

\textbf{With or Without Conditions? Equity-Efficiency Tradeoffs in Municipal Transfers in Brazil}\\
\textit{with \href{https://www.luigicaloi.com/}{Luigi Caloi}}

\medskip

\textit{Abstract:} To improve service delivery, central governments can tie intergovernmental transfers to local
