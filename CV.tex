...
\textit{Abstract:} Social scientists often use topic models to study documents produced by different authors in different contexts, even though the same broad topic may be expressed differently across environments. Existing methods typically control for this variation ex post and cannot separate global themes from environment-specific language. Their reliance on word counts creates a second problem, as concepts expressed through context, phrases, and tone may never emerge as coherent topics. We introduce the mechanistic Fixed Effects Topic Model (mFETM), which incorporates fixed effects directly into the generative process and represents documents as interpretable concepts extracted from a large language model with sparse autoencoders (SAEs). Applied to a large corpus of Glassdoor reviews, mFETM recovers topics that are more stable across observed environments without sacrificing fit. At the firm level, mFETM recovers a broader, more interpretable range of general workplace dimensions associated with top employers. A conventional model confounds these with environment-specific topics, such as technology strategy, yielding a narrower and less complete account and losing substantially more fit when environment-associated topics are removed. Finally, the SAE representation allows us to generate counterfactual reviews by steering along the estimated fixed-effect directions, holding a theme fixed while its environment changes.
